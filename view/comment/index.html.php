    ...
    <article class="p-3 border border-1 rounded mb-3" id="comment<?= $comment->getIdComment() ?>">
        ...
        <ul class="nav">
            <li class="nav-item me-2">
                <a class="nav-link btn btn-primary text-light" href="<?= sprintf("/comment/show/%d", $comment->getIdComment()) ?>">Read</a>
            </li>
            <?php
            if (isset($_SESSION['user'])) :
            ?>
                <li class="nav-item me-2">
                    <a class="nav-link btn btn-light text-dark" href="<?= sprintf("/comment/edit/%d", $comment->getIdComment()) ?>">Edit</a>
                </li>
                <li class="nav-item me-2">
